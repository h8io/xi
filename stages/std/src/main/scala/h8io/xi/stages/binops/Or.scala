      ...
      case Yield.None(leftSignal, leftOnDone) =>
        right(in) match {
          case Yield.Some(out, rightSignal, rightOnDone) =>
            Yield.Some(Right(out), leftSignal ~> rightSignal, Or.RightOnDone(leftOnDone, rightOnDone))
          case Yield.None(rightSignal, rightOnDone) =>
            Yield.None(leftSignal ~> rightSignal, Or.RightOnDone(leftOnDone, rightOnDone))
        }
    }
}
...
object Or {
  ...
  private final case class RightOnDone[-I, +LO, +RO, +E](leftOnDone: OnDone[I, LO, E], rightOnDone: OnDone[I, RO, E])
      extends OnDone[I, Either[LO, RO], E] {
