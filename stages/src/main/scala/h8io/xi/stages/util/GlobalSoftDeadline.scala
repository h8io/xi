...
object GlobalSoftDeadline {
  private[util] final case class Impl[T](now: () => Instant, deadline: Instant) extends Stage.Endo[T, Nothing] {
    self =>
  ...
  def apply[T](deadline: Instant): Stage.Endo[T, Nothing] = Impl(Instant.now _, deadline)

  def apply[T](duration: FiniteDuration): Stage.Endo[T, Nothing] =
    Impl(Instant.now _, Instant.now().plusNanos(duration.toNanos))
