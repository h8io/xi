...
object LocalSoftDeadline {
  private[util] final case class Head[T](now: () => Long, duration: Long) extends Stage.EndoWithOnDone[T, Nothing] {
  ...
  private[util] final case class Tail[T](ts: Long, head: Head[T]) extends Stage.EndoWithOnDone[T, Nothing] {
    def apply(in: T): Yield.Some[T, T, Nothing] =
