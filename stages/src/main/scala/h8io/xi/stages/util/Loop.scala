...
final case class Loop[T, +E](condition: Condition, stage: Stage.Endo[T, E]) extends Stage.SafeEndo[T, E] {
  def apply(in: T): Yield[T, T, E] = {
    @tailrec def loop(condition: Condition, stage: Stage[T, T, E], in: T, dispose: () => Unit): Yield[T, T, E] =
      ...
      } else Yield.Some(in, State.Success(stage).onDone(dispose))
    loop(condition, stage, in, { () => })
  }
}
