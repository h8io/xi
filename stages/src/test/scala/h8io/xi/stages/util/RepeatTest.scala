...
class RepeatTest
  ...
  "Repeat" should "be executed until the state is Complete" in
    forAll(Gen.zip(Gen.choose(0, 100), Gen.long)) { case (n, in) =>
      val initial = mock[Stage[Long, Option[String], Nothing]]("initial stage")
      ...
      val resultStage = mock[Stage[Long, Option[String], Nothing]]("result stage")
  ...
  it should "be executed until the state is Error" in
    forAll(Gen.zip(Gen.choose(0, 100), Arbitrary.arbitrary[Instant], Arbitrary.arbitrary[State.Error[Exception]])) {
      case (n, in, lastState) =>
        val initial = mock[Stage[Instant, Option[UUID], Exception]]("initial stage")
        val updated = genStage(n, initial, in)
        val lastYield = genYield[Instant, UUID, Exception]("last", lastState)
        (updated.apply _).expects(in).returns(lastYield)
        val resultStage = mock[Stage[Instant, Option[UUID], Exception]]("result stage")
  ...
  @tailrec private def genStage[I, O: Arbitrary, E](
      i: Int,
      stage: Stage[I, Option[O], E], in: I): Stage[I, Option[O], E] =
    if (i > 0) {
      val `yield` = genYield[I, O, E](i.toString, State.Success)
      val updated = mock[Stage[I, Option[O], E]](s"stage $i")
      (`yield`.onDone.onSuccess _).expects().returns(updated)
      (stage.apply _).expects(in).returns(`yield`)
      genStage(i - 1, updated, in)
    } else stage

  private def genYield[I, O: Arbitrary, E](id: String, state: State[E]): Yield[I, Option[O], E] =
    Gen.prob(0.9).filter(identity).sample match {
      case Some(_) => Yield.Some(Arbitrary.arbitrary[O].sample, state, mock[OnDone[I, Option[O], E]](s"onDone $id"))
      case None => Yield.None(state, mock[OnDone[I, Option[O], E]])
    }
