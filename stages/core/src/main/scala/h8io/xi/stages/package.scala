...
package object stages {
  type Alteration[-IS <: Stage[?, ?, ?], +OS <: Stage[?, ?, ?]] = IS => OS

  type Decorator[I, O, E] = Alteration[Stage[I, O, E], Stage[I, O, E]]

  implicit final class AlterationOps[-IS <: Stage[?, ?, ?], +OS <: Stage[?, ?, ?]](val alteration: Alteration[IS, OS])
      extends AnyVal {
    @inline def ∘[_IS <: Stage[?, ?, ?]](that: Alteration[_IS, IS]): Alteration[_IS, OS] =
