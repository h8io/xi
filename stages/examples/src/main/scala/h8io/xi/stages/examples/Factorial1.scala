...
object Factorial1 {
  sealed class Agg(n: Long) extends Stage.Endo[BigInt, Nothing] with OnDone[BigInt, BigInt, Nothing] {
    override def apply(in: BigInt): Yield.Some[BigInt, BigInt, Nothing] = Yield.Some(in * n, Signal.Success, this)

    override def onSuccess(): Stage[BigInt, BigInt, Nothing] = new Agg(n + 1)
  ...
  def stage(n: Int): Stage[Unit, BigInt, Nothing] = Const(BigInt.apply(1)) ~> Loop(Agg ~> Countdown(n))
}
