...
final case class Cache[-I, +O, +E](underlying: Stage[I, O, E]) extends Decorator[I, O, E] {
...
object Cache {
  private[examples] final case class Cached[-I, +O, +E](out: O, underlying: Stage[I, O, E])
      extends Decorator[I, O, E] with Fruitful[I, O, E] {
    def apply(in: I): Yield.Some[I, O, E] = Yield.Some(out, Signal.Success, this)

    override def onComplete(): Stage[I, O, E] = Cache(underlying)
    override def onError(): Stage[I, O, E] = Cache(underlying)
  }
}
