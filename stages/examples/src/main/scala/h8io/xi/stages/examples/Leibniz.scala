...
import h8io.xi.stages.decorators.{KeepLastOutput, LocalSoftDeadline, Repeat}
...
object Leibniz {
  ...
  def stage(duration: FiniteDuration) = Repeat(KeepLastOutput(LocalSoftDeadline(duration, InitialStage)))
}
